...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // 데이터베이스에 삽입할 쿼리 작성
    $stmt = $conn->prepare("INSERT INTO BOARDLOST (userID, category, title, Ddate, firstLocation, detailLocation, detailContent, path) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssssss", $userID, $category, $title, $Ddate, $firstLocation, $detailLocation, $detailContent, $path);
